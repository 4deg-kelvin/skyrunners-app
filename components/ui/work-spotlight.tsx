      ...
      <CardBody>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-ink-soft mt-1.5 text-[15px]">
              ...
              it, or stuck, first. Ask to join and the project&apos;s RE
              decides; the request is tracked, and escalates if it sits for five
              days.
            </p>
          </div>
          {counts.total > 0 ? (
            <div className="text-right">
              <SectionLabel tone="muted">Needing people</SectionLabel>
              <p className="text-ink mt-1 text-2xl font-bold">
                {counts.needingHelp + counts.unstaffed}
              </p>
            </div>
          ) : null}
        </div>
        ...
        {available.length > SPOTLIGHT_LIMIT ? (
          <p className="text-ink-muted mt-4 text-sm">
            ...
            {available.length - SPOTLIGHT_LIMIT === 1 ? "" : "s"} in the
            divisions below — every one has an <em>Ask to join</em> button on
            its own page.
          </p>
        ) : null}
      </CardBody>
