...
const deliverables: CollectionSpec<Deliverable> = {
  ...
  table: "deliverables",
  columns:
    "id, project_id, title, kind, owner_id, due_date, status, submitted_at, completed_at, confirmed_by, blocker_note, sort_order",
  identify: (d) => d.id,
  fromRow: (r) => ({
    id: r.id as string,
    projectId: r.project_id as string,
    title: r.title as string,
    kind: r.kind === "milestone" ? "milestone" : "deliverable",
    ownerId: opt(r.owner_id as string),
